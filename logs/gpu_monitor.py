...
def getGPUs(server):
    stdout = run_command(f"ssh {server} nvidia-smi --query-gpu=index,uuid,utilization.gpu,memory.total,memory.used,memory.free,driver_version,name,gpu_serial,display_active,display_mode,temperature.gpu --format=csv,noheader,nounits")
...
if __name__ == "__main__":
    servers = [f"gpu0{i+1}" for i in range(3, 8)]
    for server in servers:
        print(server)
        gpu_info = getGPUs(server)
        showUtilization(gpu_info)
        print(getAvailable(gpu_info, order='first',
                           limit=4, maxLoad=0.7, maxMemory=0.3, memoryFree=0, includeNan=False,
