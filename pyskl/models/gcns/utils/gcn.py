...
# by gzb 20220816
class unit_gcn_frame(nn.Module):
    ...
    def forward(self, x, A=None):
        ...
        if self.conv_pos == 'pre': # by gzb: N C T V -> N K C T V -> N C T V
            x = self.conv(x)
            x = x.view(n, self.num_subsets, -1, t, v) # by gzb: N C T V -> N K C T V
            #x = torch.einsum('nkctv,kvw->nctw', (x, A)).contiguous() # N K C T V -> N C T V
            x = torch.einsum('nkctv,ktw->ncwv', (x, A)).contiguous() # N K C T V -> N C T(W) V
...
class unit_gcn_local(nn.Module):
